...
function getResult(inputData) {
    ...
    if (inputData.length < 4) {
        if (inputData[0] + inputData[1] + inputData[2] === targetNumber) return inputData[0] * inputData[1] * inputData[2]
    ...
    for (let i = 0; i < sortedData.length - 2; i++) {
        ...
        while (result !== targetNumber) {
            if (secondPointer > firstPointer) {
                ...
                if (result === targetNumber) { return sortedData[i] * sortedData[firstPointer] * sortedData[secondPointer] }
            } else {
                break
            }
        }
